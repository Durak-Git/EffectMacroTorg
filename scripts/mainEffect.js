...
Hooks.on("updateCombatant", async (torgCombatant, dataFlags, dataDiff, userId) => {
    if (game.user.hasRole(4)) {
        if (dataFlags.flags.world.turnTaken) {
            ...
            for (const ef of myActor.effects) {
                await myActor.updateEmbeddedDocuments("ActiveEffect", [{ _id: ef.id, "duration.turns": ef.duration.turns - 1, "duration.rounds": ef.duration.rounds - 1 }]);
    ...
    await Dialog.confirm({
        ...
        yes: () => {
            if (skillValue > 0) {
                ...
                ui.notifications.warn(driver.name + game.i18n.localize("EffectMacroTorg.noCapacity"))
            }
        },
        no: () => { },
        render: () => { },
